...
## 7. Observability (developer aside) — optional ~1–2 min

Two dev-only surfaces pin to the corners (both hidden from production/demo builds):

- **Debug panel** (bottom-right) exposes the raw time-series payload, the routing-series mapping, and live query states. *"It makes drift between the charts and the data obvious while building."*
- **Observability panel** (bottom-left) subscribes to the frontend telemetry store and shows recent **API latencies**, the backend **request ids** (`X-Request-ID`), the **last assistant result** (`stable` / `risk_alert`), and a live **event stream**. Click **show** to expand.

Correlation is the point: *"Every API call is stamped with an `X-Request-ID` — generated by the backend or preserved if the client sends one — and echoed on the response. The frontend reads it back, so a latency I see in this panel maps to one structured JSON log line on the backend by the same id."*

Inspect the backend logs directly to close the loop — each line is a JSON object with `event`, `request_id`, `duration_ms`, and safe metadata (never raw clinical text):

```bash
# In the terminal running uvicorn, requests log as structured JSON, e.g.:
{"ts":"…","level":"INFO","logger":"caretrace.http","message":"http_request",
 "request_id":"…","event":"http_request","method":"GET","path":"/api/runs/…",
 "status_code":200,"duration_ms":4.1}
```

The observability panel can also be enabled in a production/demo build with the safe opt-in flag `NEXT_PUBLIC_OBSERVABILITY=1` (off by default).
...
- **"Tested and hardened"** — 107 backend + 41 frontend tests, a Playwright demo-path E2E suite, and a GitHub Actions quality gate; plus per-widget error boundaries, an immutable-decision 409 guard, and lazy-loaded charts.
- **"Observable, without heavy infra"** — every request carries a correlation id and is timed and logged as structured JSON; key flows emit safe telemetry; a dev-only panel ties a UI action back to the exact backend log — no external telemetry stack.
